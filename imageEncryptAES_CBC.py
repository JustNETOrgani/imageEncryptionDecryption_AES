...
# Note: It is without message authentication (unauthenticated encryption).
import sys
import time
import cv2
import hashlib
...
def encrypt(dirPath, image_original, initialization_vector_size, password):
    ...
    image_name = f"encryptedImg_AES_CBC_{str(int(time.time()))}.bmp"
...
def decrypt(dirPath, imageEncrypted, initialization_vector_size, password):
...
def main():
    ...
    userChoice = int(input("""
        Select operation to execute:
            1. Encrypt an image using AES_CBC
            2. Decrypt the image using AES_CBC
            3. Exist program
        """))
    if userChoice == 1:
        ...
        if imgStatus == True: # To be improved to enforce check that it is encrypted image file.
            ...
            decrypt(decryptedImgDir, readEncImgFile, initialization_vector_size, keyAsPassword)
        else: 
